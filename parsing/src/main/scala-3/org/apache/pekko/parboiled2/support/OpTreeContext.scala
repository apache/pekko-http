...
import org.apache.pekko.parboiled2.support.hlist.HList
...
class OpTreeContext(parser: Expr[Parser])(using Quotes) {
  ...
  sealed abstract class NonTerminalOpTree extends OpTree {
    def bubbleUp(e: Expr[org.apache.pekko.parboiled2.Parser#TracingBubbleException], start: Expr[Int]): Expr[Nothing]
    ...
    def render(wrapped: Boolean): Expr[Boolean] =
      if (wrapped) '{
        ...
        try ${ renderInner('start, wrapped) }
          ...
          case e: org.apache.pekko.parboiled2.Parser#TracingBubbleException => ${ bubbleUp('e, 'start) }
        }
      }
    ...
    def renderInner(start: Expr[Int], wrapped: Boolean): Expr[Boolean] =
      collector.withCollector { coll =>
        '{
          ...
          def rec(mark: org.apache.pekko.parboiled2.Parser.Mark): org.apache.pekko.parboiled2.Parser.Mark = {
  ...
  case class Times(
      ...
      withMinMax: MinMaxSupplier,
      collector: Collector,
      separator: Separator) extends WithSeparator {
    def withSeparator(sep: Separator) = copy(separator = sep)
    def ruleTraceNonTerminalKey = withMinMax((min, max) => '{ org.apache.pekko.parboiled2.RuleTrace.Times($min, $max) })
      ...
      collector.withCollector { coll =>
        withMinMax { (minE, maxE) =>
          '{
            ...
            @ _root_.scala.annotation.tailrec
            def rec(count: Int, mark: org.apache.pekko.parboiled2.Parser.Mark): Boolean = {
              val matched = ${ op.render(wrapped) }
              if (matched) {
                ${ coll.popToBuilder }
                if (count < max) ${
  ...
  case class SemanticPredicate(flagTree: Expr[Boolean]) extends TerminalOpTree {
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.SemanticPredicate }
  ...
  case class CharMatch(charTree: Expr[Char]) extends TerminalOpTree {
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.CharMatch($charTree) }
    override def renderInner(wrapped: Boolean): Expr[Boolean] = {
      val unwrappedTree = '{
            ...
            if ($parser.cursorChar == $ch) {
              ...
              try $parser.__registerMismatch()
                ...
                case org.apache.pekko.parboiled2.Parser.StartTracingException =>
                  ...
                  $parser.__bubbleUp(
                    NonTerminal(org.apache.pekko.parboiled2.RuleTrace.StringMatch($stringTree), -${ Expr(ix) }) :: Nil,
                    org.apache.pekko.parboiled2.RuleTrace.CharMatch($ch))
              }
            }
          }
        } else '{ true }
  ...
  case class IgnoreCaseChar(charTree: Expr[Char]) extends TerminalOpTree {
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.IgnoreCaseChar($charTree) }
            ...
            if (_root_.java.lang.Character.toLowerCase($parser.cursorChar) == $ch) {
              ...
              try $parser.__registerMismatch()
                ...
                case org.apache.pekko.parboiled2.Parser.StartTracingException =>
                  import org.apache.pekko.parboiled2.RuleTrace._
                  $parser.__bubbleUp(
                    NonTerminal(org.apache.pekko.parboiled2.RuleTrace.IgnoreCaseString($stringTree),
                      -${ Expr(ix) }) :: Nil,
                    org.apache.pekko.parboiled2.RuleTrace.IgnoreCaseChar($ch))
              }
            }
          }
        } else '{ true }
  ...
  private case class CharPredicateMatch(predicateTree: Expr[CharPredicate])
    ...
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.CharPredicateMatch($predicateTree) }
  ...
  case class AnyOf(stringTree: Expr[String]) extends TerminalOpTree {
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.AnyOf($stringTree) }
  ...
  case class NoneOf(stringTree: Expr[String]) extends TerminalOpTree {
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.NoneOf($stringTree) }
  ...
  case object ANY extends TerminalOpTree {
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.ANY }
  ...
  case class CharacterRange(lowerBound: Expr[Char], upperBound: Expr[Char]) extends TerminalOpTree {
    def ruleTraceTerminal = '{ org.apache.pekko.parboiled2.RuleTrace.CharRange($lowerBound, $upperBound) }
  ...
  case class Fail(stringExpr: Expr[String]) extends OpTree {
    def render(wrapped: Boolean): Expr[Boolean] = '{ throw new org.apache.pekko.parboiled2.Parser.Fail($stringExpr) }
  }
