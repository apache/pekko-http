...
trait DynamicRuleDispatchMacro { self: DynamicRuleDispatch.type =>
  ...
  inline def apply[P <: Parser, L <: HList](
      inline ruleNames: String*): (DynamicRuleDispatch[P, L], immutable.Seq[String]) =
    ${ DynamicRuleDispatch.__create[P, L]('ruleNames) }

  import scala.quoted._
  def __create[P <: Parser: Type, L <: HList: Type](
      ruleNames: Expr[Seq[String]])(using Quotes): Expr[(DynamicRuleDispatch[P, L], immutable.Seq[String])] = {
    ...
    '{
      ...
      val drd =
        new org.apache.pekko.parboiled2.DynamicRuleDispatch[P, L] {
