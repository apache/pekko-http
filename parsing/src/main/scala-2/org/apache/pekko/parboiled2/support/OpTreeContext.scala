...
trait OpTreeContext[OpTreeCtx <: ParserMacros.ParserContext] {
  ...
  sealed abstract class NonTerminalOpTree extends OpTree {
    ...
    def render(wrapped: Boolean): Tree =
      if (wrapped) q"""
        ...
        catch { case e: org.apache.pekko.parboiled2.Parser#TracingBubbleException => $bubbleUp }"""
  ...
  sealed abstract class TerminalOpTree extends OpTree {
    ...
    final def render(wrapped: Boolean): Tree =
      if (wrapped) q"""
        try ${renderInner(wrapped)}
        catch { case org.apache.pekko.parboiled2.Parser.StartTracingException => $bubbleUp }"""
      else renderInner(wrapped)
  ...
  sealed abstract class PotentiallyNamedTerminalOpTree(arg: Tree) extends TerminalOpTree {
    ...
    override def bubbleUp =
      callName(arg) match {
        case Some(name) =>
          q"__bubbleUp(org.apache.pekko.parboiled2.RuleTrace.NonTerminal(org.apache.pekko.parboiled2.RuleTrace.Named($name), 0) :: Nil, $ruleTraceTerminal)"
  ...
  val opTreePF: PartialFunction[Tree, OpTree] = {
    ...
    case call @ (Apply(_, _) | Select(_, _) | Ident(_) | TypeApply(_, _)) =>
      RuleCall(Right(call),
        Literal(Constant(callName(call).getOrElse(c.abort(call.pos, "Illegal rule call: " + call)))))
  }
  ...
  case class Cut(lhs: OpTree, rhs: OpTree) extends DefaultNonTerminalOpTree {
    ...
    def renderInner(wrapped: Boolean): Tree = q"""
      ...
      if (matched) {
        ...
        if (!matched) throw org.apache.pekko.parboiled2.Parser.CutError
        true
  ...
  case class CharMatch(charTree: Tree) extends TerminalOpTree {
    def ruleTraceTerminal = q"org.apache.pekko.parboiled2.RuleTrace.CharMatch($charTree)"
        ...
        if (ix < s.length) {
          ...
          q"""if (cursorChar == $ch) {
            ...
            try __registerMismatch()
              ...
              case org.apache.pekko.parboiled2.Parser.StartTracingException =>
                import org.apache.pekko.parboiled2.RuleTrace._
  ...
  case class IgnoreCaseChar(charTree: Tree) extends TerminalOpTree {
    def ruleTraceTerminal = q"org.apache.pekko.parboiled2.RuleTrace.IgnoreCaseChar($charTree)"
  ...
  case class CharPredicateMatch(predicateTree: Tree) extends PotentiallyNamedTerminalOpTree(predicateTree) {
    def ruleTraceTerminal = q"org.apache.pekko.parboiled2.RuleTrace.CharPredicateMatch($predicateTree)"
  ...
  case class AnyOf(stringTree: Tree) extends TerminalOpTree {
    def ruleTraceTerminal = q"org.apache.pekko.parboiled2.RuleTrace.AnyOf($stringTree)"
  ...
  case class NoneOf(stringTree: Tree) extends TerminalOpTree {
    def ruleTraceTerminal = q"org.apache.pekko.parboiled2.RuleTrace.NoneOf($stringTree)"
  ...
  case class ZeroOrMore(op: OpTree, collector: Collector, separator: Separator = null) extends WithSeparator {
    ...
    def renderInner(wrapped: Boolean): Tree = {
      ...
      @_root_.scala.annotation.tailrec def rec(mark: org.apache.pekko.parboiled2.Parser.Mark): org.apache.pekko.parboiled2.Parser.Mark = {
        val matched = ${op.render(wrapped)}
        if (matched) {
  ...
  case class Times(op: OpTree, init: Tree, collector: Collector, separator: Separator) extends WithSeparator {
    ...
    def ruleTraceNonTerminalKey = q"..$inits; org.apache.pekko.parboiled2.RuleTrace.Times(min, max)"
    ...
    def renderInner(wrapped: Boolean): Tree = {
      ...
      @_root_.scala.annotation.tailrec def rec(count: Int, mark: org.apache.pekko.parboiled2.Parser.Mark): Boolean = {
        val matched = ${op.render(wrapped)}
        if (matched) {
          ${collector.popToBuilder}
      ...
      if (wrapped) {
        val base = op match {
          case x: TerminalOpTree =>
            q"org.apache.pekko.parboiled2.RuleTrace.NotPredicate.Terminal(${x.ruleTraceTerminal})"
          case x: RuleCall => q"org.apache.pekko.parboiled2.RuleTrace.NotPredicate.RuleCall(${x.calleeNameTree})"
          case x: StringMatch =>
            q"""org.apache.pekko.parboiled2.RuleTrace.NotPredicate.Named("\"" + ${x.stringTree} + '"')"""
          case x: IgnoreCaseString =>
            q"""org.apache.pekko.parboiled2.RuleTrace.NotPredicate.Named("\"" + ${x.stringTree} + '"')"""
          case x: Named => q"org.apache.pekko.parboiled2.RuleTrace.NotPredicate.Named(${x.stringTree})"
          case _        => q"org.apache.pekko.parboiled2.RuleTrace.NotPredicate.Anonymous"
        }
        q"""
        var matchEnd = 0
        try $unwrappedTree || __registerMismatch()
        catch {
          case org.apache.pekko.parboiled2.Parser.StartTracingException => __bubbleUp {
            org.apache.pekko.parboiled2.RuleTrace.NotPredicate($base, matchEnd - cursor)
          }
        }"""
      } else unwrappedTree
    }
            ...
            case q"(..$args => $body)" =>
              def rewrite(tree: Tree): Tree =
                tree match {
                  ...
                  case x if isSubClass(resultTypeTree.tpe, "org.apache.pekko.parboiled2.Rule") => expand(x, wrapped)
                  case x                                                                       => q"__push($x)"
                }
              val valDefs = args
                .zip(argTypeTrees)
      ...
      hlTree match {
        ...
        case q"support.this.HListable.fromHList[$t]" =>
          @tailrec def rec(t: Type, result: List[Tree] = Nil): List[Tree] =
            t match { // TODO: how can we use type quotes here, e.g. tq"akka.parboiled2.support.hlist.HNil"?
  ...
  case class CharacterRange(lowerBound: Char, upperBound: Char) extends TerminalOpTree {
    def ruleTraceTerminal = q"org.apache.pekko.parboiled2.RuleTrace.CharRange($lowerBound, $upperBound)"
        ...
        tree match {
          ...
          case q"(..$args => $body)" =>
            def rewrite(tree: Tree): Tree =
              tree match {
                case Block(statements, res)                                               => block(statements, rewrite(res))
                case x if isSubClass(actionType.last, "org.apache.pekko.parboiled2.Rule") => expand(x, wrapped)
                case x                                                                    => q"__push($x)"
              }
  ...
  case class Fail(stringTree: Tree) extends OpTree {
    def render(wrapped: Boolean): Tree = q"throw new org.apache.pekko.parboiled2.Parser.Fail($stringTree)"
  }

  case class Named(op: OpTree, stringTree: Tree) extends DefaultNonTerminalOpTree {
    def ruleTraceNonTerminalKey = q"org.apache.pekko.parboiled2.RuleTrace.Named($stringTree)"
  ...
  lazy val HListConsTypeSymbol = c.mirror.staticClass("org.apache.pekko.parboiled2.support.hlist.$colon$colon")
  lazy val HNilTypeSymbol = c.mirror.staticClass("org.apache.pekko.parboiled2.support.hlist.HNil")
