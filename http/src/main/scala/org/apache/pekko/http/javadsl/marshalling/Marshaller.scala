...
import pekko.http.javadsl.model.{ ContentType, HttpHeader, HttpResponse, MediaType, RequestEntity, StatusCode }
import pekko.http.scaladsl
import pekko.http.scaladsl.marshalling
...
object Marshaller {
  ...
  def fromScala[A, B](scalaMarshaller: marshalling.Marshaller[A, B]): Marshaller[A, B] =
    new Marshaller()(scalaMarshaller)
  ...
  def downcast[A, B1, B2 <: B1](m: Marshaller[A, B2], target: Class[B1]): Marshaller[A, B1] =
    m.asInstanceOf[Marshaller[A, B1]]
  ...
  def wrapEntity[A, C, E <: RequestEntity](
      f: function.Function[C, A], m: Marshaller[A, E], mediaType: MediaType): Marshaller[C, RequestEntity] = {
    val scalaMarshaller = m.asScalaCastOutput
  ...
  implicit def asScalaToResponseMarshaller[T](
      m: Marshaller[T, pekko.http.javadsl.model.HttpResponse]): ToResponseMarshaller[T] =
    m.asScala.map(_.asScala)

  implicit def asScalaEntityMarshaller[T](m: Marshaller[T, pekko.http.javadsl.model.RequestEntity])
      : pekko.http.scaladsl.marshalling.Marshaller[T, pekko.http.scaladsl.model.RequestEntity] =
    m.asScala.map(_.asScala)
}
...
class Marshaller[-A, +B] private (implicit val asScala: marshalling.Marshaller[A, B]) {
  ...
  @InternalApi
  private[pekko] def asScalaCastOutput[C]: marshalling.Marshaller[A, C] =
    asScala.asInstanceOf[marshalling.Marshaller[A, C]]
