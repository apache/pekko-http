...
class Http2ServerSpec extends Http2SpecWithMaterializer("""
  ...
  "The Http/2 server implementation" should {
    "support simple round-trips" should {
      abstract class SimpleRequestResponseRoundtripSetup extends TestSetup with RequestResponseProbes {
        def requestResponseRoundtrip(
            ...
            expectedResponseHeaderBlock: ByteString): Unit = {
      ...
      "GET request in one HEADERS frame".inAssertAllStagesStopped(new SimpleRequestResponseRoundtripSetup {
        requestResponseRoundtrip(
          ...
          expectedRequest =
            HttpRequest(HttpMethods.GET, "http://www.example.com/", protocol = HttpProtocols.`HTTP/2.0`),
      ...
      "GET request in one HEADERS and one CONTINUATION frame".inAssertAllStagesStopped(
      ...
      "GET request in one HEADERS and two CONTINUATION frames".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          val headerBlock = HPackSpecExamples.C41FirstRequestWithHuffman
          val fragment1 = headerBlock.take(8) // must be grouped by octets
      ...
      "parse headers to modeled headers".inAssertAllStagesStopped(new TestSetup with RequestResponseProbes {
        ...
        val expectedRequest = HttpRequest(
          ...
          uri = "http://www.example.com/",
          headers =
            Vector(`Cache-Control`(`no-cache`), `Cache-Control`(`max-age`(1000)), `Access-Control-Allow-Origin`.`*`),
          protocol = HttpProtocols.`HTTP/2.0`)
        val streamId = 1
    ...
    "support HEAD requests" should {
      abstract class HeadRequestSetup extends TestSetup with RequestResponseProbes {
        val TheStreamId = 1
        def sendHeadRequest(): HttpRequest = {
          network.sendRequest(TheStreamId,
            HttpRequest(HttpMethods.HEAD, "https://www.example.com/", protocol = HttpProtocols.`HTTP/2.0`))
          user.expectRequest()
        }
      }

      "deliver the request to the handler with an empty entity".inAssertAllStagesStopped(new HeadRequestSetup {
        val request = sendHeadRequest()
        request.method shouldBe HttpMethods.HEAD
        request.entity shouldBe HttpEntity.Empty
      ...
      "render content-length for a response entity with a known length but no bytes".inAssertAllStagesStopped(
        new HeadRequestSetup {
          sendHeadRequest()
          user.emitResponse(TheStreamId,
    ...
    def requestTests(minCollectStrictEntityBytes: Int) = {
      abstract class RequestEntityTestSetup extends TestSetup with RequestResponseProbes {
        override def settings: ServerSettings =
          super.settings.mapHttp2Settings(_.withMinCollectStrictEntitySize(minCollectStrictEntityBytes))
      ...
      abstract class WaitingForRequestData extends RequestEntityTestSetup {
        lazy val request = HttpRequest(method = HttpMethods.POST, uri = "https://example.com/upload",
          protocol = HttpProtocols.`HTTP/2.0`)
      ...
      s"support stream for request entity data (min-collect-strict-entity-bytes = $minCollectStrictEntityBytes)" should {
        ...
        "handle content-length and content-type of incoming request".inAssertAllStagesStopped(new WaitingForRequest(
          HttpRequest(
            ...
            entity =
              HttpEntity(ContentTypes.`application/json`, 1337, Source.repeat("x").take(1337).map(ByteString(_))),
            protocol = HttpProtocols.`HTTP/2.0`)) {
        ...
        if (minCollectStrictEntityBytes != 0)
          "not dispatch request at all if RST is received before any data".inAssertAllStagesStopped(
        ...
        "close the stream when we receive a RST after we have half-closed ourselves as well".inAssertAllStagesStopped(
          new WaitingForRequestData {
            ...
            network.sendDATA(streamId = TheStreamId, endStream = false,
              ByteString(0, 0, 0, 0, 0x10, 0x22, 0x0E) ++ ByteString.fromString("GreeterService"))
        ...
        if (minCollectStrictEntityBytes == 0)
          "not fail the whole connection when one stream is RST twice".inAssertAllStagesStopped(
            new WaitingForRequestData {
        ...
        "backpressure until request entity stream is read (don't send out unlimited WINDOW_UPDATE before)".inAssertAllStagesStopped(
          new WaitingForRequestData {
        ...
        "fail if more data is received than connection window allows".inAssertAllStagesStopped(
          new WaitingForRequestData {
        ...
        "collect Strict entity for single DATA frame".inAssertAllStagesStopped(new TestSetup
        ...
        "collect Strict entity for multiple DATA frames if min-collect-strict-entity-size is set accordingly".inAssertAllStagesStopped(
          new TestSetup with RequestResponseProbes {
            override def settings: ServerSettings =
              super.settings.mapHttp2Settings(_.withMinCollectStrictEntitySize(10))
            network.sendRequestHEADERS(1, HttpRequest(HttpMethods.POST, entity = HttpEntity("abcde")),
              endStream = false)
            user.requestIn.ensureSubscription()
            user.requestIn.expectNoMessage(100.millis) // don't expect request yet
        ...
        "create streamed entity for multiple DATA frames if min-collect-strict-entity-size is too low".inAssertAllStagesStopped(
          new TestSetup with RequestResponseProbes {
            override def settings: ServerSettings =
              super.settings.mapHttp2Settings(_.withMinCollectStrictEntitySize(10))
            network.sendRequestHEADERS(1, HttpRequest(HttpMethods.POST, entity = HttpEntity("abcde")),
        ...
        def sendOutConnectionLevelWindowUpdate(singleDataFrame: Boolean) =
          s"eventually send out WINDOW_UPDATE for dispatched data (singleDataFrame = $singleDataFrame)".inAssertAllStagesStopped(
            new WaitingForRequestData {
              override def settings: ServerSettings =
                super.settings.mapHttp2Settings(
                  _.withIncomingConnectionLevelBufferSize(Http2Protocol.InitialWindowSize)) // set to initial size
        ...
        def handleEarlyWindowUpdateCorrectly(endStreamSeen: Boolean) =
          s"handle WINDOW_UPDATE correctly when received before started sending out response and while receiving request data (seen endStream = $endStreamSeen)".inAssertAllStagesStopped(
            new WaitingForRequestData {
              ...
              val response = HttpResponse(entity = HttpEntity(ContentTypes.`application/octet-stream`,
                Source.fromPublisher(entityDataOut)))
              user.emitResponse(TheStreamId, response)
              network.expectDecodedHEADERS(streamId = TheStreamId, endStream = false) shouldBe response.withEntity(
                HttpEntity.Empty.withContentType(ContentTypes.`application/octet-stream`))
    ...
    "support streaming for sending response entity data" should {
      ...
      "keep sending entity data when WINDOW_UPDATE is received intermediately".inAssertAllStagesStopped(
      ...
      "keep sending entity data when data is chunked into small bits".inAssertAllStagesStopped(
        new WaitingForResponseDataSetup {
      ...
      "handle RST_STREAM while data is waiting in outgoing stream buffer".inAssertAllStagesStopped(
        new WaitingForResponseDataSetup {
          val data1 = ByteString("abcd")
          entityDataOut.sendNext(data1)
      ...
      "handle WINDOW_UPDATE correctly when received before started sending out response".inAssertAllStagesStopped(
      ...
      "distribute increases to SETTINGS_INITIAL_WINDOW_SIZE to streams correctly while sending out response".inAssertAllStagesStopped(
        new WaitingForResponseDataSetup {
      ...
      "cancel entity data source when peer sends RST_STREAM before entity is subscribed".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          val theRequest = HttpRequest(protocol = HttpProtocols.`HTTP/2.0`)
      ...
      "send data frame with exactly the number of remaining connection-level window bytes even when chunk is bigger than that".inAssertAllStagesStopped(
        new WaitingForResponseDataSetup {
      ...
      "backpressure response entity stream until WINDOW_UPDATE was received".inAssertAllStagesStopped(
        new WaitingForResponseDataSetup {
    ...
    "support multiple concurrent substreams" should {
      ...
      "send two responses concurrently".inAssertAllStagesStopped(new TestSetup with RequestResponseProbes {
        ...
        val response1 = HttpResponse(entity = HttpEntity(ContentTypes.`application/octet-stream`,
          Source.fromPublisher(entity1DataOut)))
        user.emitResponse(1, response1)
        network.expectDecodedHEADERS(streamId = 1, endStream = false) shouldBe response1.withEntity(
          HttpEntity.Empty.withContentType(ContentTypes.`application/octet-stream`))

        def sendDataAndExpectOnNet(outStream: TestPublisher.Probe[ByteString], streamId: Int, dataString: String,
        ...
        val response2 = HttpResponse(entity = HttpEntity(ContentTypes.`application/octet-stream`,
          Source.fromPublisher(entity2DataOut)))
        user.emitResponse(3, response2)
        network.expectDecodedHEADERS(streamId = 3, endStream = false) shouldBe response2.withEntity(
          HttpEntity.Empty.withContentType(ContentTypes.`application/octet-stream`))
      ...
      "receiving RST_STREAM for one of two sendable streams".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          val theRequest = HttpRequest(protocol = HttpProtocols.`HTTP/2.0`)
          network.sendRequest(1, theRequest)
          user.expectRequest() shouldBe theRequest
      ...
      "close substreams when connection is shutting down".inAssertAllStagesStopped(
        StreamTestKit.assertAllStagesStopped(new TestSetup with RequestResponseProbes {
          ...
          val responseEntity =
            HttpEntity(ContentTypes.`application/octet-stream`, Source.fromPublisher(responseEntityProbe))
    ...
    "respect flow-control" should {
      ...
      "reject WINDOW_UPDATE for stream with zero increment with PROTOCOL_ERROR".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "backpressure incoming frames when outgoing control frame buffer fills".inAssertAllStagesStopped(
        new TestSetup with HandlerFunctionSupport {
          override def settings: ServerSettings =
            super.settings.mapHttp2Settings(_.withOutgoingControlFrameBufferSize(1))
    ...
    "respect settings" should {
      ...
      "received non-zero length payload Settings with ACK flag (invalid 6.5)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          /*
      ...
      "received SETTINGS frame with a length other than a multiple of 6 octets (invalid 6_5)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "received SETTINGS_MAX_FRAME_SIZE should cause outgoing DATA to be chunked up into at-most-that-size parts ".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          ...
          val tooLargeResponse =
            HttpResponse(entity = HttpEntity(ContentTypes.`application/octet-stream`, tooLargeEntity))
      ...
      "received SETTINGS_MAX_CONCURRENT_STREAMS should limit the number of streams".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "not limit response streams even when the client send a SETTINGS_MAX_CONCURRENT_STREAMS".inAssertAllStagesStopped(
        new TestSetup(
          Setting(SettingIdentifier.SETTINGS_MAX_CONCURRENT_STREAMS, 1)) with RequestResponseProbes {
          def openStream(streamId: Int) = network.sendHEADERS(streamId, endStream = false, endHeaders = true,
            HPackSpecExamples.C41FirstRequestWithHuffman)
      ...
      "react on invalid SETTINGS_INITIAL_WINDOW_SIZE with FLOW_CONTROL_ERROR".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
    ...
    "enforce settings" should {
      ...
      "reject new substreams when exceeding SETTINGS_MAX_CONCURRENT_STREAMS".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          def maxStreams: Int = 16
          override def settings: ServerSettings =
            super.settings.mapHttp2Settings(_.withMaxConcurrentStreams(maxStreams))
      ...
      "reject new substreams when exceeding SETTINGS_MAX_CONCURRENT_STREAMS (with closed streams in between)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
    ...
    "support low-level features" should {
      ...
      "respond to invalid (not 0x0 streamId) PING with GOAWAY PROTOCOL_ERROR (spec 6_7)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "respond to invalid (length smaller than 8) with GOAWAY FRAME_SIZE_ERROR (spec 6_7)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "respond to invalid (length larger than 8) with GOAWAY FRAME_SIZE_ERROR (spec 6_7)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
    ...
    "respect the substream state machine" should {
      ...
      "reject other frame than HEADERS/PUSH_PROMISE in idle state with connection-level PROTOCOL_ERROR (5.1)".inAssertAllStagesStopped(
        new SimpleRequestResponseRoundtripSetup {
      ...
      "reject mid-stream HEADERS with endStream = false".inAssertAllStagesStopped(
        new SimpleRequestResponseRoundtripSetup {
          network.sendHEADERS(1, endStream = false, endHeaders = true, HPackSpecExamples.C41FirstRequestWithHuffman)
          network.sendHEADERS(1, endStream = false, endHeaders = true, HPackSpecExamples.C41FirstRequestWithHuffman)
          network.expectGOAWAY()
      ...
      "reject substream creation for streams invalidated by skipped substream IDs".inAssertAllStagesStopped(
        new SimpleRequestResponseRoundtripSetup {
    ...
    "support for configurable pings" should {
      "send pings when there is an active but slow stream to server" in StreamTestKit.assertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          override def settings: ServerSettings = {
            val default = super.settings
      ...
      "send pings when there is an active but slow stream from server" in StreamTestKit.assertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          ...
          val response1 = HttpResponse(entity = HttpEntity(ContentTypes.`application/octet-stream`,
            Source.fromPublisher(responseStream)))
          user.emitResponse(1, response1)
          network.expectDecodedHEADERS(streamId = 1, endStream = false) shouldBe response1.withEntity(
            HttpEntity.Empty.withContentType(ContentTypes.`application/octet-stream`))
          network.expectNoBytes(250.millis) // no data for 500ms interval should trigger ping (but client counts from emitting last frame, so it's not really 500ms here)
      ...
      "send GOAWAY when ping times out" in StreamTestKit.assertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          override def settings: ServerSettings = {
            val default = super.settings
    ...
    "support graceful shutdown" should {
      "immediately shutdown when no requests are pending".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "close after current requests have been processed (while request data was outstanding)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "close after current requests have been processed (when response was outstanding)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
      ...
      "close after current requests have been processed (while response data was outstanding)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          network.sendRequest(1, HttpRequest())
          user.expectRequest()
      ...
      "finally close after deadline (while closing request stream)".inAssertAllStagesStopped(
        new TestSetup with RequestResponseProbes {
          network.sendRequestHEADERS(1, HttpRequest(), endStream = false)
          val req = user.expectRequest()
          val p = ByteStringSinkProbe(req.entity.dataBytes)
      ...
      "finally close after deadline (while cancelling response stream)".inAssertAllStagesStopped(new TestSetup
        with RequestResponseProbes {
        network.sendRequest(1, HttpRequest())
        user.expectRequest()
        ...
        val dataStream = TestPublisher.probe[ByteString]()
        user.emitResponse(1,
          HttpResponse(entity = HttpEntity(ContentTypes.`application/octet-stream`, Source.fromPublisher(dataStream))))
        network.expectDecodedHEADERS(1, endStream = false)
        dataStream.sendNext(ByteString("abc"))
        network.expectDATA(1, endStream = false, ByteString("abc"))
