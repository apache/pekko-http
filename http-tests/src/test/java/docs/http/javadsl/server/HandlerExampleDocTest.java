...
public class HandlerExampleDocTest extends JUnitRouteTest {
  ...
  public void testSimpleHandler() {
    ...
    class TestHandler {
      ...
      Route createRoute() {
        return concat(get(() -> handlerString), post(() -> path("abc", () -> handlerResponse)));
      }
    }
    ...
    r.run(HttpRequest.POST("/test")).assertStatusCode(StatusCodes.NOT_FOUND);
    ...
    class TestHandler {
      ...
      final Route multiplyXAndYParam =
          parameter(
              ...
              "x",
              x ->
                  parameter(StringUnmarshallers.INTEGER, "y", y -> complete("x * y = " + (x * y))));

      final Route pathMultiply =
          path(integerSegment().slash(integerSegment()), (x, y) -> complete("x * y = " + (x * y)));
            ...
            get(
                () ->
                    pathPrefix(
                        ...
                        () ->
                            concat(
                                path("multiply", () -> multiplyXAndYParam),
                                pathPrefix("path-multiply", () -> pathMultiply),
    ...
    class TestHandler {
      ...
      Route paramXY(BiFunction<Integer, Integer, Route> inner) {
        return parameter(
            StringUnmarshallers.INTEGER,
            "x",
            x -> parameter(StringUnmarshallers.INTEGER, "y", y -> inner.apply(x, y)));
      }
      ...
      Route createRoute() {
        return concat(
            get(() -> pathPrefix("calculator", () -> concat(multiplyAsyncRoute, addAsyncRoute))));
      }
    }
