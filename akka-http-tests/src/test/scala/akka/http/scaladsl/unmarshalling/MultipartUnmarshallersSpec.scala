  ...
  "The MultipartUnmarshallers." should {
    ...
    "multipartGeneralUnmarshaller should correctly unmarshal 'multipart/*' content with" should {
      ...
      "an implicitly typed part (without headers) (Strict)" in {
        Unmarshal(HttpEntity(
          ...
          ByteString("""--XYZABC
                       ...
                       |--XYZABC--""".stripMarginWithNewline(lineFeed)))).to[Multipart.General] should haveParts(
          Multipart.General.BodyPart.Strict(HttpEntity(ContentTypes.`text/plain(UTF-8)`,
            "Perfectly fine part content.")))
      }
      ...
      "two different parts" in {
        Unmarshal(HttpEntity(
          ...
          Multipart.General.BodyPart.Strict(HttpEntity(ContentTypes.`text/plain(UTF-8)`,
            s"first part, with a trailing newline${lineFeed}")),
          Multipart.General.BodyPart.Strict(
      ...
      "a full example (Strict)" in {
        Unmarshal(HttpEntity(
          ...
          Multipart.General.BodyPart.Strict(HttpEntity(ContentTypes.`text/plain(UTF-8)`,
            "first part, implicitly typed")),
          Multipart.General.BodyPart.Strict(HttpEntity(`application/octet-stream`,
      ...
      "a full example (Strict) containing a CR in the body part" in {
        Unmarshal(HttpEntity(
          ...
          ByteString("""preamble and
                       ...
                       |--12345--
                       |epilogue and
                       |more epilogue""".stripMarginWithNewline(lineFeed).replaceAll("INSERT_CR_HERE", "\r")))).to[
          Multipart.General] should haveParts(
          Multipart.General.BodyPart.Strict(HttpEntity(ContentTypes.`text/plain(UTF-8)`,
            "first part, impli\rcitly typed")),
          Multipart.General.BodyPart.Strict(HttpEntity(`application/octet-stream`,
            ByteString("second part, explicitly typed"))))
      }
      ...
      "a boundary with spaces" in {
        Unmarshal(HttpEntity(
          ...
          ByteString("""--simple boundary
                       |--simple boundary--""".stripMarginWithNewline(lineFeed)))).to[
          Multipart.General] should haveParts(
          Multipart.General.BodyPart.Strict(HttpEntity.empty(ContentTypes.`text/plain(UTF-8)`)))
      }
    ...
    "multipartGeneralUnmarshaller should reject illegal multipart content with" should {
      ...
      "a stray boundary" in {
        Await.result(
          ...
          1.second.dilated).getMessage shouldEqual "Illegal multipart boundary in message content"
      }
        ...
        Await.result(
          Unmarshal(HttpEntity(
            ...
            ByteString("""---
                       ...
                       |-----""".stripMarginWithNewline(lineFeed))))
            .to[Multipart.General].failed,
          1.second.dilated).getMessage shouldEqual "Illegal character ' ' in header name"
      }
      "a missing header-separating CRLF (in Default entity)" in {
        ...
        Await.result(Unmarshal(HttpEntity.Default(contentType, content.length, Source(byteStrings)))
            .to[Multipart.General]
            .flatMap(_.toStrict(1.second.dilated)).failed,
          1.second.dilated).getMessage shouldEqual "Illegal character ' ' in header name"
      }
    ...
    "multipartByteRangesUnmarshaller should correctly unmarshal multipart/byteranges content with two different parts" in {
      Unmarshal(HttpEntity(
        ...
        Multipart.ByteRanges.BodyPart.Strict(ContentRange(0, 2, 26),
          HttpEntity(ContentTypes.`text/plain(UTF-8)`, "ABC")),
        Multipart.ByteRanges.BodyPart.Strict(ContentRange(23, 25, 26),
    ...
    "multipartFormDataUnmarshaller should correctly unmarshal 'multipart/form-data' content" should {
      ...
      "with one element" in {
        Unmarshal(HttpEntity(
          ...
          ByteString("""--XYZABC
                       ...
                       |test@there.com
                       |--XYZABC--""".stripMarginWithNewline(lineFeed)))).to[Multipart.FormData] should haveParts(
          Multipart.FormData.BodyPart.Strict("email",
      ...
      "with a file" in {
        Unmarshal {
          ...
          Multipart.FormData.BodyPart.Strict("email",
            HttpEntity(`application/octet-stream`, ByteString("test@there.com"))),
          Multipart.FormData.BodyPart.Strict("userfile", HttpEntity(`application/pdf`, ByteString("filecontent")),
