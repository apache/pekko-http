# Agent Rules for Apache Pekko HTTP
...
- HTTP/2 changes must consider compatibility with HTTP/1.1 fallback paths.
- Marshaller changes (spray-json, xml, jackson) must be tested independently.
...
- For HTTP tests, use the appropriate testkit (httpTestkit for Akka HTTP testkit-style, httpTestkitMunit for MUnit-style).
...
- For Java DSL (`javadsl`) APIs, ensure the API surface is Java-friendly: avoid Scala-specific types, default parameters, and lower type bounds.
...
- For JDK-specific behavior, test with both JDK 17 and JDK 21 where relevant.
