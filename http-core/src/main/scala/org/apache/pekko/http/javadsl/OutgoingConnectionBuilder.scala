...
trait OutgoingConnectionBuilder {
   ...
   * the requests go out, so requests need a [[pekko.http.javadsl.model.RequestResponseAssociation]]
   * which Pekko HTTP will carry over to the corresponding response for a request.
   */
  @ApiMayChange
  def http2WithFallback(): Flow[HttpRequest, HttpResponse, CompletionStage[OutgoingConnection]]
