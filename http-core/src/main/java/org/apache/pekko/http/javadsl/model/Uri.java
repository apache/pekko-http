...
public abstract class Uri {
  ...
  public abstract Query query(
      Charset charset, org.apache.pekko.http.scaladsl.model.Uri.ParsingMode mode);
   ...
   *
   * <p>Characters that are not within the range described at
   * https://tools.ietf.org/html/rfc3986#section-3.4 should be percent-encoded. Characters that are
   * in that range may or may not be percent-encoded, and depending on how the query string is
   * parsed this might be relevant: for example, when interpreting the query string as 'key=value'
   * pairs you could use the percent-encoded '=' ('%22) to include a '=' in the key or value.
   *
   * <p>When characters are encountered that are outside of the RFC3986 range they are automatically
   ...
   * pairs you could use the percent-encoded '=' ('%22) to include a '=' in the key or value.
   *
   * @param strict depending on the 'strict' flag, characters outside of the range allowed by
   *     RFC3986 will either cause a `IllegalUriException` or be automatically percent-encoded. Be
   *     aware that relying on automatic percent-encoding is usually a programming error.
   */
  ...
  public static final org.apache.pekko.http.scaladsl.model.Uri.ParsingMode STRICT =
      UriJavaAccessor.pmStrict();
  public static final org.apache.pekko.http.scaladsl.model.Uri.ParsingMode RELAXED =
      UriJavaAccessor.pmRelaxed();

  /** Creates a default Uri to be modified using the modification methods. */
  public static final Uri EMPTY =
      new JavaUri(org.apache.pekko.http.scaladsl.model.Uri.Empty$.MODULE$);
  ...
  public static Uri create(
      String uri, org.apache.pekko.http.scaladsl.model.Uri.ParsingMode parsingMode) {
    return new JavaUri(
        org.apache.pekko.http.scaladsl.model.Uri.apply(
            ParserInput$.MODULE$.apply(uri), parsingMode));
  }
  ...
  public static Uri create(
    ...
    return new JavaUri(
        org.apache.pekko.http.scaladsl.model.Uri.apply(
            ParserInput$.MODULE$.apply(uri), charset, parsingMode));
  }
