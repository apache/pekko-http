jQuery(document).ready(function ($) {
    ...
    function initOldVersionWarnings($) {
        $.get("//akka.io/versions.json", function (akkaHTTPVersionsData) {
            var site = extractCurrentPageInfo();
            if (site.v === 'snapshot') {
                showSnapshotWarning(site)
            } else {
                var matchingMinor =
                    Object.keys(akkaHTTPVersionsData[site.p])
                        .find(function(s) { return site.v.startsWith(s) })
                if (matchingMinor) {
                    showVersionWarning(site, akkaHTTPVersionsData, matchingMinor)
                }
            }
        })
    }

    function getInstead(akkaHTTPVersionsData, project, instead) {
        var found, proj
        if (Array.isArray(instead)) {
            found = akkaHTTPVersionsData[instead[0]][instead[1]]
            proj = instead[0]
        } else {
            found = akkaHTTPVersionsData[project][instead]
            proj = project
        }
    ...
    function showVersionWarning(site, akkaHTTPVersionsData, series) {
        var version = site.v,
            seriesInfo = akkaHTTPVersionsData[site.p][series]


        if (versionWasAcked(site.p, version)) {
            // hidden for a day
        } else if (seriesInfo.outdated) {
            var instead = getInstead(akkaHTTPVersionsData, site.p, seriesInfo.instead)
