...
public class HostDirectivesExamplesTest extends JUnitRouteTest {
  ...
  public void testHostPredicate() {
    // #host2
    final Route shortOnly =
        host(hostname -> hostname.length() < 10, () -> complete(StatusCodes.OK));

    testRoute(shortOnly)
        .run(HttpRequest.GET("/").addHeader(Host.create("short.com")))
        .assertStatusCode(StatusCodes.OK);

    testRoute(shortOnly)
        .run(HttpRequest.GET("/").addHeader(Host.create("verylonghostname.com")))
  ...
  public void testExtractHost() {
    ...
    final Route route = extractHost(hn -> complete("Hostname: " + hn));

    testRoute(route)
        .run(HttpRequest.GET("/").addHeader(Host.create("company.com", 9090)))
  ...
  public void testMatchAndExtractHost() {
    ...
    final Route hostPrefixRoute =
        host(Pattern.compile("api|rest"), prefix -> complete("Extracted prefix: " + prefix));

    final Route hostPartRoute =
        host(
            Pattern.compile("public.(my|your)company.com"),
            captured -> complete("You came through " + captured + " company"));
    ...
    testRoute(route)
        .run(HttpRequest.GET("/").addHeader(Host.create("api.company.com")))
        .assertStatusCode(StatusCodes.OK)
        .assertEntity("Extracted prefix: api");

    testRoute(route)
        .run(HttpRequest.GET("/").addHeader(Host.create("public.mycompany.com")))
        .assertStatusCode(StatusCodes.OK)
