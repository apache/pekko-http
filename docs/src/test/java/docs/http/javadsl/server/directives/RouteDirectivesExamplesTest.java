...
public class RouteDirectivesExamplesTest extends JUnit5RouteTest {
  ...
  public void testShowRedirectOnCompleteWithTerms() {
    ...
    final Function<RequestContext, CompletionStage<RouteResult>> route =
        rc ->
            // #red-impl
            rc.completeWith(
                HttpResponse.create().withStatus(redirectionType).addHeader(Location.create(uri))
                // #red-impl
                );
  }
  ...
  public void testComplete() {
    ...
    // tests:
    testRoute(route)
        .run(HttpRequest.GET("/a"))
        .assertStatusCode(StatusCodes.OK)
        .assertEntity("foo");

    testRoute(route).run(HttpRequest.GET("/b")).assertStatusCode(StatusCodes.OK).assertEntity("OK");

    testRoute(route)
        .run(HttpRequest.GET("/c"))
        .assertStatusCode(StatusCodes.CREATED)
        .assertEntity("bar");

    testRoute(route)
        .run(HttpRequest.GET("/d"))
        .assertStatusCode(StatusCodes.CREATED)
        .assertEntity("bar");

    testRoute(route)
        .run(HttpRequest.GET("/e"))
        .assertStatusCode(StatusCodes.CREATED)
        .assertHeaderExists(ContentType.create(ContentTypes.TEXT_PLAIN_UTF8))
        .assertEntity("bar");

    testRoute(route)
        .run(HttpRequest.GET("/f"))
        .assertStatusCode(StatusCodes.CREATED)
        .assertHeaderExists(ContentType.create(ContentTypes.TEXT_PLAIN_UTF8))
        .assertEntity("bar");

    testRoute(route)
        .run(HttpRequest.GET("/g"))
        .assertStatusCode(StatusCodes.OK)
        .assertEntity("baz");
    // #complete
  }
  ...
  public void testRedirect() {
    ...
    // tests:
    testRoute(route).run(HttpRequest.GET("/foo/")).assertEntity("yes");
